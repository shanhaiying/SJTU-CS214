...
\footnotesize{\color{blue}$*$ Name:David Wang  \quad Student ID:518021910976 \quad Email: davidwang200099@sjtu.edu.cn}
\end{center}
...
\begin{enumerate}
    ...
    \begin{solution}
        Maintain a 2-dimension array called $dp$, where $dp[i][j]$ means the minimal width of the books put horizontally when the total thickness of the books put vertically is $j$, when trying to put the first $i$ books into the shelf.
        
        The $i$-th book can be put either vertically or horizontally, so we need to choose to make $dp[i][j]$ as small as possible.
        
        Some $dp[i][j]$ is not the right answer because it exceeds $j$, which means that the total width of horizontal books is larger than the total thickness of the vertical books.
        
        So we need to find the minimal $j$ such that $dp[i][j]\le j$, which means the total width of horizontal books does not exceed the total thickness of the vertical books.
        
        The result of \texttt{Data-P1.txt} is 2542.
    ...
    \end{enumerate}
    \begin{solution}
        The edit distance between the two sequences is 7615.
    \end{solution}
